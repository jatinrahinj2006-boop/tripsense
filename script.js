const apiKey = "951e4032e2499e80890a35fda4554fdb";
...
async function fetchCityData(cityName) {
  try {
    ...
    const geoRes = await fetch(`https://api.openweathermap.org/geo/1.0/direct?q=${encodeURIComponent(cityName)}&limit=5&appid=${apiKey}`);
...
cityInput?.addEventListener('input', (e) => {
  ...
  debounceTimer = setTimeout(async () => {
    try {
      const res = await fetch(`https://api.openweathermap.org/geo/1.0/direct?q=${encodeURIComponent(value)}&limit=5&appid=${apiKey}`);
