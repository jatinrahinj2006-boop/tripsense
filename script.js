...
async function fetchCityData(cityName) {
  try {
    ...
    const geoRes = await fetch(`https://tripsense-weather.jatinrahinj2006.workers.dev?endpoint=geo&params=q=${encodeURIComponent(cityName)}&limit=5`);
...
async function fetchWeatherData(cityData) {
  try {
    ...
    const [aqiRes, weatherRes, forecastRes, uvRes] = await Promise.all([
      fetch(`https://tripsense-weather.jatinrahinj2006.workers.dev?endpoint=air_pollution&params=lat=${lat}&lon=${lon}`),
      fetch(`https://tripsense-weather.jatinrahinj2006.workers.dev?endpoint=weather&params=lat=${lat}&lon=${lon}&units=metric`),
      fetch(`https://tripsense-weather.jatinrahinj2006.workers.dev?endpoint=forecast&params=lat=${lat}&lon=${lon}&units=metric`),
      fetch(`https://tripsense-weather.jatinrahinj2006.workers.dev?endpoint=uvi&params=lat=${lat}&lon=${lon}`)
    ]);
...
cityInput?.addEventListener('input', (e) => {
  ...
  debounceTimer = setTimeout(async () => {
    try {
      const res = await fetch(`https://tripsense-weather.jatinrahinj2006.workers.dev?endpoint=geo&params=q=${encodeURIComponent(value)}&limit=5`);
